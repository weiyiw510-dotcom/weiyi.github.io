...
uint8_t Key_GetNum(void)
{
	...
	if ((xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED) && (App_KeyQueue != 0))
	{
		if (xQueueReceive(App_KeyQueue, &Temp, 0) == pdTRUE)
		{
			return Temp;
		}
		return 0;
...
void Key_Tick(void)
{
	...
	if(Count>=20)
	{
		...
		if(PreState!=0&&CurrentState==0)
		{
			if ((xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED) && (App_KeyQueue != 0))
			{
				uint8_t key = PreState;
				xQueueSend(App_KeyQueue, &key, 0);
			}
			else
			{
				Key_Num=PreState;
