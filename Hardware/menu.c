...
//电池电量采样任务调用，避免界面刷新时长时间占用CPU
void Battery_Update(void)
{
	uint32_t sum = 0;
	signed portBASE_TYPE locked = pdFALSE;
	...
	if (App_BatteryMutex != 0)
	{
		locked = xSemaphoreTake(App_BatteryMutex, 20 / portTICK_RATE_MS);
	}
	...
	if (locked == pdTRUE)
	{
		xSemaphoreGive(App_BatteryMutex);
	}
}
...
void Show_Battery(void)
{
	...
	OLED_ShowNum(85,4,capacity,3,OLED_6X8);
	OLED_ShowChar(103,4,'%',OLED_6X8);
	
	if(capacity==100)OLED_ShowImage(110,0,16,16,Battery);
	else if(capacity>=10&&capacity<100)
	{
		OLED_ShowImage(110,0,16,16,Battery);
		OLED_ClearArea((112+capacity/10),5,(10-capacity/10),6);
